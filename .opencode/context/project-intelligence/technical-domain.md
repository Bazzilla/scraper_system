...
**Orchestratore**: `src/orchestrator.py` — `run(config_path, output_path, db_path)`, crea `output/` automaticamente, inietta tickers e risolve cache_path/history_path, applica manual overrides, arricchisce matrice con source runtime; logging talkative su console: nel loop logga `▶ [name] start` / `✓ [name] done` con elapsed, e ogni modulo scraper logga cosa scrapa (URL/ticker/cache) tramite il decorator `@log_scrape` in `fetch_utils.py`; `run.py` configura `logging.basicConfig(level=INFO)` così l'output raggiunge la console (prima silenziato perché il basicConfig viveva solo in `scheduler.py`)
